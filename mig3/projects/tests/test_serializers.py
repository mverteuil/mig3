...
def test_project_summary_serializer(project):
    ...
    serializer = serializers.ProjectSummarySerializer(instance=project, context=context)
...
def test_target_summary_serializer(target):
    ...
    context = {"request": mock.Mock(name="request", GET={})}
    serializer = serializers.TargetSummarySerializer(instance=target, context=context)
    assert serializer.data is not None
...
def test_version_write_serializer(db):
    ...
    serializer = serializers.VersionWriteSerializer(data=data)
